    ...
    ($shop_preorder==1 && $pkt_arr[0]==$tomorrow_date && $pkt_arr[1]>=$shop_open && $pkt_arr[1]<=$shop_close) ){
        ...
        try {
            $charge = \Stripe\Charge::create([
                'amount' => $payamount * 100, // Stripe expects amount in cents
                'currency' => 'inr',
                'source' => $stripe_token,
                'description' => 'CafeConnect Order Payment'
            ]);
            ...
            if($charge->status == 'succeeded'){
                ...
                $payment_query = "INSERT INTO payment (p_amount,p_method,p_status,p_reference) VALUES ({$amt},'{$payment_method}','PAID','{$payment_reference}');\n";
