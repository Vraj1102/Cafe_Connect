...
$payamount = $_GET["amount"] ?? 0;

if(empty($pickuptime) || $payamount <= 0){
    header("location: cust_cart.php");
    exit(1);
}

// Validate shop and pickup time
...
($shop_preorder==1 && $pkt_arr[0]==$tomorrow_date && $pkt_arr[1]>=$shop_open && $pkt_arr[1]<=$shop_close)){
    ...
    // Payment successful via Stripe - Create order
    $payment_type = "CRDC"; // Default to credit card
